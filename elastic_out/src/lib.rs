...
fn append_demo_code() {
  ...
  title.set_text_content(Some("Elastic Out Tween..."));
...
fn demo_code() -> &'static str {
  ...
  let mut tweener = Tweener::new_at(start, end, duration, ElasticOut, in_a_second);
...
fn appened_restart_link() {
  ...
  link.set_attribute("href", "/elastic-out.html").expect("failed to set href attribute to link");
...
fn elastic_out() -> Result<(), JsValue> {
    ...
    let mut tweener = Tweener::new_at(start - (BOX_SIZE / 2) , end - (BOX_SIZE / 2), duration, ElasticOut, in_a_second);
