...
fn demo_code() -> &'static str {
  ...
  let mut tweener = Tweener::new_at(start, end, duration, ElasticIn, in_a_second);
...
fn appened_restart_link() {
  ...
  link.set_attribute("href", "/elastic-in.html").expect("failed to set href attribute to link link");
...
fn elastic_in() -> Result<(), JsValue> {
    ...
    let mut tweener = Tweener::new_at(start - (BOX_SIZE / 2) , end - (BOX_SIZE / 2), duration, ElasticIn, in_a_second);
